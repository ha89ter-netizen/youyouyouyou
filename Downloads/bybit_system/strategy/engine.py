...
class StrategyEngine:
    ...
    def run_once(self):
        ...
        for symbol in self.cfg.symbols:
            try:
                self._process_symbol(symbol, balance, positions)
            except Exception:
    ...
    def _sync_closed_trades(self):
        ...
        for symbol in self.cfg.symbols:
            ...
            for trade in open_trades:
                ...
                self.journal.log_exit(trade["order_link_id"], exit_price, pnl_usdt)
                self.risk_manager.record_closed_pnl(pnl_usdt)
                logger.info(
                    "%s: сделка %s закрыта на бирже, PnL=%.2f USDT (сверено по цене входа %.4f)",
                    symbol, trade["order_link_id"], pnl_usdt, trade["entry_price"],
                )
    ...
    def _process_symbol(self, symbol: str, balance: float, positions: list):
        ...
        funding_rate = self._load_latest_funding(symbol)
        ...
        if existing_position is not None:
            ...
            self._manage_exit(symbol, existing_position, final_signal, trend)
            return
    ...
    def _manage_exit(self, symbol: str, position: dict, final_signal: Signal, trend: Optional[str]):
        """
        Exit Manager: закрывает позицию ДО срабатывания фиксированного SL/TP,
        если появились веские основания считать её больше не оправданной:
        1. Разворотный сигнал -- reconciled-сигнал (уже прошедший через
           _reconcile и trend filter, то есть достаточно надёжный) указывает
           на ПРОТИВОПОЛОЖНОЕ позиции направление.
        2. Смена старшего тренда (EMA50/200) против направления позиции --
           крупная смена режима рынка, даже если краткосрочные индикаторы
           ещё не развернулись явно.

        Закрытие делается обычным reduceOnly-ордером; фактический PnL и
        обновление журнала подхватит _sync_closed_trades на следующем цикле
        (сверка по цене входа + времени, как и для SL/TP-закрытий).
        """
        ...
        if final_signal.action == Action.OPEN_LONG and position_direction == "short":
            close_reason = f"Разворотный сигнал против позиции: {final_signal.reason}"
        elif final_signal.action == Action.OPEN_SHORT and position_direction == "long":
            close_reason = f"Разворотный сигнал против позиции: {final_signal.reason}"
        elif trend is not None and trend != "neutral" and trend != position_direction:
            close_reason = (
                f"Старший тренд (EMA50/200) сменился на {trend}, "
                f"позиция ({position_direction}) теперь открыта против него"
            )
    ...
    def _apply_trend_filter(self, signal: Signal, trend: Optional[str], symbol: str) -> Signal:
        """
    ...
    def _load_recent_candles(self, symbol: str, limit: int = 100) -> Optional[pd.DataFrame]:
        ...
        try:
            rows = (
                ...
                .filter(Candle.symbol == symbol, Candle.interval == "15")
    ...
    def _load_latest_funding(self, symbol: str) -> Optional[float]:
        session = self.db.get_session()
        try:
            row = (
    ...
    def _load_oi_trend(self, symbol: str, limit: int = 20) -> Optional[dict]:
        ...
        try:
            ...
            return {"current": values[-1], "change_pct": change_pct}
        finally:
            session.close()
